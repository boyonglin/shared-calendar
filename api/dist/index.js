...
function verifyToken(req) {
    ...
    if (!finalToken)
        return null;
    try {
        return jwt.verify(finalToken, JWT_SECRET);
    }
    catch {
        return null;
    }
...
function getEncryptionKey() {
    if (!ENCRYPTION_KEY) {
        throw new Error("ENCRYPTION_KEY is required for iCloud integration. Please set it in your environment.");
    }
...
function sanitizeInput(input, maxLength = AI_MAX_INPUT_LENGTH) {
...
function buildAIPrompt(params) {
    ...
    lines.push("", `Please write a ${tone} invitation message for this event.`, "The message should be ready to send via email or chat.", "Keep it concise but clear.", "", "Output only plain text. Do not include any HTML tags, markdown formatting, or code blocks.");
    return lines.join("\n");
}
// =============================================================================
// Route Handlers
...
async function handleGoogleCallback(req, res) {
    ...
    if (error) {
        return res.redirect(`${CLIENT_URL}?auth=error&message=${encodeURIComponent(error)}`);
    }
    if (!code) {
    ...
    try {
      ...
      ON CONFLICT(user_id) DO UPDATE SET
        ...
        return res.redirect(`${CLIENT_URL}?auth=error&message=${encodeURIComponent(message)}`);
    }
}
...
async function handleGetUser(userId, res) {
    const client = getTursoClient();
...
async function handleGetAllEvents(req, res, userId) {
    ...
    for (const account of result.rows) {
        if (account.provider === "google" && account.access_token) {
            try {
                ...
                console.error(`Error fetching Google events for ${account.user_id}:`, err);
            }
        }
        ...
        if (account.provider === "outlook" &&
            ...
            try {
                ...
                const calendarsResponse = await fetch(`${ONECAL_API_BASE}/calendars/${endUserAccountId}`, {
                ...
                for (const calendar of calendars) {
                    ...
                    const eventsResponse = await fetch(`${ONECAL_API_BASE}/events/${endUserAccountId}/${calendar.id}?${eventsParams}`, { headers: { "x-api-key": ONECAL_API_KEY } });
                ...
                console.error(`Error fetching Outlook events for ${account.user_id}:`, err);
            }
        }
        ...
        if (account.provider === "icloud" && account.encrypted_password) {
            try {
                ...
                console.error(`Error fetching iCloud events for ${account.user_id}:`, err);
            }
        }
    }
...
async function handleICloudConnect(req, res) {
    const user = verifyToken(req);
    if (!user) {
        return res.status(401).json({ error: "Unauthorized" });
    }
    ...
    try {
        ...
        return res.status(401).json({
            error: "Failed to verify iCloud credentials. Please check your Apple ID and App-Specific Password.",
        });
    }
...
async function handleICloudStatus(req, res) {
    const user = verifyToken(req);
    if (!user) {
        return res.status(401).json({ error: "Unauthorized" });
    }
...
async function handleRemoveICloud(req, res, userId) {
    const user = verifyToken(req);
    if (!user) {
        return res.status(401).json({ error: "Unauthorized" });
    }
...
async function handleOutlookStatus(req, res) {
    const user = verifyToken(req);
    if (!user) {
        return res.status(401).json({ error: "Unauthorized" });
    }
...
async function handleRemoveOutlook(req, res, userId) {
    const user = verifyToken(req);
    if (!user) {
        return res.status(401).json({ error: "Unauthorized" });
    }
...
async function handleOutlookAuth(req, res) {
    ...
    res.setHeader("Set-Cookie", `outlook_auth_state=${stateToken}; HttpOnly; Secure; SameSite=Lax; Path=/; Max-Age=600`);
...
async function handleOutlookCallback(req, res) {
    ...
    res.setHeader("Set-Cookie", "outlook_auth_state=; HttpOnly; Secure; SameSite=Lax; Path=/; Max-Age=0");
    if (!endUserAccountId || typeof endUserAccountId !== "string") {
        return res.redirect(`${CLIENT_URL}?auth=error&message=missing_endUserAccountId`);
    }
    ...
    if (!primaryUserId) {
        return res.redirect(`${CLIENT_URL}?auth=error&message=invalid_or_expired_state`);
    }
    if (!ONECAL_API_KEY) {
        return res.redirect(`${CLIENT_URL}?auth=error&message=onecal_not_configured`);
    }
    try {
        // Fetch account info from OneCal
        const response = await fetch(`${ONECAL_API_BASE}/endUserAccounts/${endUserAccountId}`, {
            headers: { "x-api-key": ONECAL_API_KEY },
        });
        if (!response.ok) {
            const error = await response.text();
            console.error("Failed to fetch OneCal account:", error);
            return res.redirect(`${CLIENT_URL}?auth=error&message=onecal_fetch_failed`);
        }
      ...
      ON CONFLICT(user_id) DO UPDATE SET
        ...
        return res.redirect(`${CLIENT_URL}?auth=success&provider=outlook&code=${authCode}`);
    }
    catch (error) {
...
function generateFriendColor(email) {
    let hash = 0;
...
function extractFriendName(metadata, email) {
    if (metadata) {
        try {
...
async function handleSyncPending(req, res) {
    ...
    for (const conn of pendingResult.rows) {
        ...
        if (friendAccountResult.rows.length > 0) {
            const friendUserId = friendAccountResult.rows[0].user_id;
...
async function handleAddFriend(req, res) {
    ...
    const userEmails = userAccountResult.rows
        .map((row) => row.external_email?.toLowerCase())
        .filter((email) => !!email);
    ...
    const friendAccount = friendAccountResult.rows.length > 0 ? friendAccountResult.rows[0] : null;
    const status = friendAccount ? "requested" : "pending";
    const friendUserId = friendAccount ? friendAccount.user_id : null;
    // Get current user's email for reverse connection
    const primaryUserResult = await client.execute({
        sql: "SELECT external_email FROM calendar_accounts WHERE user_id = ?",
        args: [user.userId],
    });
    const primaryUserEmail = primaryUserResult.rows.length > 0
        ? primaryUserResult.rows[0].external_email
        : null;
    try {
...
async function handleAcceptFriend(req, res, friendId) {
    const user = verifyToken(req);
    if (!user) {
        return res.status(401).json({ error: "Unauthorized" });
    }
...
async function handleRejectFriend(req, res, friendId) {
    const user = verifyToken(req);
    if (!user) {
        return res.status(401).json({ error: "Unauthorized" });
    }
...
async function handleGetFriendEvents(req, res, friendId) {
    const user = verifyToken(req);
    if (!user) {
        return res.status(401).json({ error: "Unauthorized" });
    }
    ...
    const connection = connectionResult.rows[0];
    const friendUserId = connection.friend_user_id;
    ...
    for (const account of accountsResult.rows) {
        if (account.provider === "google" && account.access_token) {
            try {
                ...
                console.error(`Error fetching Google events for friend ${friendUserId}:`, err);
            }
        }
        // Fetch Outlook events via OneCal
        if (account.provider === "outlook" &&
            ...
            try {
                ...
                console.error(`Error fetching Outlook events for friend ${friendUserId}:`, err);
            }
        }
        // Fetch iCloud events via CalDAV
        if (account.provider === "icloud" && account.encrypted_password) {
            try {
                    ...
                    for (const obj of objects) {
                        if (obj.data) {
                            try {
                                ...
                                // Skip malformed calendar objects
                                console.error(`Error parsing iCloud calendar object for friend ${friendUserId}:`, parseErr);
                            }
                        }
                    }
                }
                ...
                console.error(`Error fetching iCloud events for friend ${friendUserId}:`, err);
            }
        }
    }
    return res.status(200).json(allEvents);
...
// AI Handlers
// =============================================================================
async function handleDraftInvitation(req, res) {
    const user = verifyToken(req);
    if (!user) {
        return res.status(401).json({ error: "Unauthorized" });
    }
    const { title, description, start, end, attendees, location, tone, geminiApiKey, } = req.body || {};
    if (!title || !start || !end) {
        return res
            .status(400)
    ...
    try {
        ...
        return res.status(500).json({
            error: "Failed to generate invitation draft. Please check your API key and try again.",
        });
    }
}
// =============================================================================
...
export default async function handler(req, res) {
    ...
    try {
        ...
        const outlookDeleteMatch = path.match(/^\/api\/calendar\/outlook\/([^/]+)$/);
