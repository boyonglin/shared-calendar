...
try {
  ...
  new Database(":memory:");
  console.log(
    "[Check-SQLite] better-sqlite3 is compatible with current Node.js version.",
  );
} catch (e) {
  const isMismatch =
  ...
  if (isMismatch) {
    console.log(
      "[Check-SQLite] Node version mismatch detected. Rebuilding better-sqlite3...",
    );
    try {
      ...
      console.error(
        "[Check-SQLite] Failed to rebuild better-sqlite3:",
        rebuildError,
      );
      process.exit(1);
    }
