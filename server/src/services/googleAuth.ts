...
export const googleAuthService = {
    ...
    handleCallback: async (code: string) => {
        ...
        // Get user info
        console.log('3. Fetching user info...');
        const oauth2 = google.oauth2({ version: 'v2', auth: oauth2Client });
        const { data: userInfo } = await oauth2.userinfo.get();
        console.log('4. User info received:', userInfo.email);
    ...
    getCalendarEvents: async (userId: string) => {
        ...
        oauth2Client.on('tokens', (tokens) => {
            if (tokens.access_token) {
                const updateStmt = db.prepare('UPDATE calendar_accounts SET access_token = ?, updated_at = CURRENT_TIMESTAMP WHERE user_id = ?');
                updateStmt.run(tokens.access_token, userId);
            }
            if (tokens.refresh_token) {
                const updateStmt = db.prepare('UPDATE calendar_accounts SET refresh_token = ?, updated_at = CURRENT_TIMESTAMP WHERE user_id = ?');
        ...
        // Fetch events similar to client logic
        const now = new Date();
        const timeMin = now.toISOString();
        const timeMax = new Date(now.setDate(now.getDate() + 28)).toISOString(); // 4 weeks
